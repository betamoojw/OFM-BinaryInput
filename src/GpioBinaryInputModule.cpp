...
void GpioBinaryInputModule::setup()
{
    ...
    for (uint8_t i = 0; i < MIN(BI_ChannelCount, OPENKNX_BI_GPIO_COUNT); i++)
    {
        ...
        openknxGPIOModule.pinMode(_gpioPins[i], INPUT_PULLUP);
#else // OPENKNX_BI_ONLEVEL == HIGH
        openknxGPIOModule.pinMode(_gpioPins[i], INPUT_PULLDOWN);
#endif

#if defined(OPENKNX_BI_PULSE) && OPENKNX_BI_PULSE != -1
        openknxGPIOModule.pinMode(OPENKNX_BI_PULSE, OUTPUT);
#endif
...
void GpioBinaryInputModule::processHardwareInputs()
{
    ...
    openknxGPIOModule.digitalWrite(OPENKNX_BI_PULSE, true);
    ...
    for (uint8_t i = 0; i < MIN(BI_ChannelCount, OPENKNX_BI_GPIO_COUNT); i++)
    {
        if (_channels[i]->isActive())
            _channels[i]->setHardwareState(openknxGPIOModule.digitalRead(_gpioPins[i]) == OPENKNX_BI_ONLEVEL);
    }

#if defined(OPENKNX_BI_PULSE) && OPENKNX_BI_PULSE != -1
    openknxGPIOModule.digitalWrite(OPENKNX_BI_PULSE, false);
